...
const RankedMovies = () => {
  ...
  return (
    <Container>
      <Title>Top Ranked Indian Movies</Title>
      <Content>
        {movies.map((movie) => (
          <Wrap key={movie.id} onClick={() => handleMovieClick(movie.id)}>
            <CardPulseBorder>
...
const Wrap = styled.div`
  position: relative;
  margin-right: 30px; /* Consistent margin */
  cursor: pointer;
  flex: 0 0 auto; /* Ensures items maintain their width */
